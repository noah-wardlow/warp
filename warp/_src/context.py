...
class Module:
    ...
    def _compile(
        ...
        # The CUDA <12.9 optimisation-level warning is meaningless on the CPU
        # path (LLVM controls -O on cpu) and on the HIP path (we drive hipcc,
        # which respects -O at every level). Suppress it for both.
...
class Device:
    ...
    def __init__(self, runtime, alias, ordinal=-1, is_primary=False, context=None):
        ...
        if self.ordinal == -1:
            ...
            # check whether our NVRTC / HIPRTC can generate device code for this
            # architecture. HIPRTC handles every gfx9xx target we currently
            # support, so the answer there is unconditionally True; on CUDA we
            # consult the static nvrtc_supported_archs list.
            if self.is_hip:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.core.wp_cuda_compile_program.argtypes = [
                ...
                ctypes.c_char_p,  # arch (sm_NN / compute_NN / gfx9NN)
                ctypes.c_char_p,  # arch_suffix ("", "a", "f")
                ctypes.c_char_p,  # include_dir
...
def capture_begin(
    ...
    # HIP / ROCm graph capture is not yet mature in ROCm 7.x (mempool pointer
    # remapping bugs, event timing inside graphs, etc.). Skip the capture and
    # let the caller fall back to direct stream submission. Returning False
    # rather than raising lets ScopedCapture transparently no-op on HIP --
    # see warp/_src/utils.py ScopedCapture.__enter__.
    if device.is_hip:
        return False
