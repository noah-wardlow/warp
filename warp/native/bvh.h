...
// CUDA's __ldg uses the read-only data path; HIP maps it to an ordinary load.
// AABB queries also load the candidate bounds into locals up front so the
// short-circuit overlap test compiles to a single predicate chain instead of a
// branch per component. The eager loads stay gated to the AABB instantiations
// because the extra live registers measurably hurt the heavier sphere/capsule
// instantiations.
