...
// On the HIP device-compile path the tex handle is reinterpreted as
// hipTextureObject_t. Even though we currently no-op texture sampling on HIP
// (CDNA3 has __HIP_NO_IMAGE_SUPPORT), keep the helper so the sampler templates
// have a consistent surface to call into.
